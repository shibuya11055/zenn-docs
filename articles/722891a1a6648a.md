...
# letとlet!
RSpecを書いたことがある方はおそらく誰もがご存知の`let`ですが、`let!`との使い分けってどうされていますか？
...
こちらについて「なぜなのか」という疑問が生まれ、今回willnetさんにいくつかの質問させていただきました。

# まず実装を追う
rspec-core 2.14.8
https://github.com/rspec/rspec-core/blob/main/lib/rspec/core/memoized_helpers.rb#L306
...
追ってみると`describe`実行時の処理の中で`subclass`が実行されていました。
```ruby
...
`subclass`の中では`MemoizedHelpers.define_helpers_on`が実行されていて、
```ruby
...
```
その中では親クラスに`module_for`でreturnされた`LetDefinitions`がincludeされています。
...
`user`は参照されていませんが予め作成される必要があるため`let!`にしている例です。
...
仕組みを理解したうえで「**letとlet!どちらでも良い場合はlet!**」に納得することができました。
...
`before`との使い分けもはっきりしたので、今後テストを書く時には明確な答えを持ってメソッドを選べるでしょう。
